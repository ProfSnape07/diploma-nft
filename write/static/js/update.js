async function validateUpdateDiploma() {
    let flag = true;
...
async function update() {
    if (account) {
        ...
        if (networkId !== chainIdOfDeployedContract) {
            ...
            if (await validateUpdateDiploma()) {
                ...
                const tx = await contractWithSigner.updateDiploma(certificateNumber, studentName, enrolmentNumber, program, specialisation, gpa, dateOfGraduation, certificateURI);
                const txHash = tx.hash;
                const explorerLinkWithTxHash = explorerLink + txHash;

                const myElement = document.getElementById("message");
                myElement.style.color = "blue";
                myElement.innerHTML = `Successfully sent to network with TX Hash: <a href="${explorerLinkWithTxHash}" target="_blank" rel="noopener noreferrer">${txHash}</a><br> Note: Only office can update a Diploma.<br>`;
